...
namespace MapHack
{
    ...
    {
        [Header("Camera/View Settings")] [Tooltip("Camera used to request streamed resources")] [SerializeField]
        ...
        [Tooltip("Direction you are facing")] [SerializeField] [Range(0, 360.0f)]
        ...
        [Tooltip("Whether to determine streaming LOD based on distance, instead of altitude")] [SerializeField]
        private bool m_streamingLodBasedOnDistance = false;

        [Header("Theme Settings")]
        [Tooltip(
            "Directory within the Resources folder to look for materials during runtime. Default is provided with the package")]
        [SerializeField]
        private string m_materialDirectory = "WrldMaterials/";

        [Tooltip("The material to override all landmarks with. Uses standard diffuse if null.")] [SerializeField]
        private Material m_overrideLandmarkMaterial = null;

        [Tooltip(
            "Set to true to use the default mouse & keyboard/touch controls, false if controlling the camera by some other means.")]
        [SerializeField]
        private bool m_useBuiltInCameraControls = true;

        [Header("Collision Settings")] [Tooltip("Set to true for Terrain collisions")] [SerializeField]
        private bool m_terrainCollisions = false;

        [Tooltip("Set to true for Road collision")] [SerializeField]
        private bool m_roadCollisions = false;

        [Tooltip("Set to true for Building collision")] [SerializeField]
        ...
        void SetupApi()
        {
            ...
            catch (InvalidApiKeyException)
            {
                ...
                EditorUtility.DisplayDialog(
                    ...
                    string.Format(
                        "Please enter a valid WRLD API Key (see the WrldMap script on GameObject \"{0}\" in the Inspector).",
                        gameObject.name),
                    "OK");
#endif
                throw;
